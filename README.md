...
## Деплой на сервер (193.42.124.206)

Аудит сервера, нормализация (чтобы проекты друг другу не мешали) и перенос кода с GitHub — пошагово в [docs/SERVER_NORMALIZE_AND_DEPLOY.md](docs/SERVER_NORMALIZE_AND_DEPLOY.md). Кратко: подключиться по SSH, выполнить `scripts/audit-server.sh`, привести к одному каталогу и systemd-сервису, затем `git clone`/`git pull` и запуск через `scripts/molotok-bot.service`.
